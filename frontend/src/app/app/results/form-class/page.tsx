...
import { FORM_CLASS_ASSESSMENTS } from "@/lib/assessments";
...
export default function FormClassResultsPage() {
  const [levels, setLevels] = useState<ClassLevelNav[]>([]);
  const [arms, setArms] = useState<ClassArm[]>([]);
  const [terms, setTerms] = useState<Term[]>([]);
  const [subjects, setSubjects] = useState<Subject[]>([]);
  const [formArms, setFormArms] = useState<number[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const [levelList, armData, termData, subjectData, meData] = await Promise.all([
          loadClassLevels(),
          ...
          isTeacher
            ? apiJson<{ positions?: Position[] }>("/api/auth/me/")
        ...
        const teacherFormArms = (meData.positions || [])
          .filter((p) => p.position === "form_teacher" && p.is_active && p.class_arm)
          .map((p) => Number(p.class_arm));

        const activeTerm = termList.find((t) => t.is_active) ?? termList[0];
        const initial: Record<number, CardState> = {};
        for (const level of levelList) {
          let levelArms = armList.filter((a) => a.class_level === level.id);
          if (isTeacher && !isAdminish) {
            levelArms = levelArms.filter((a) => teacherFormArms.includes(a.id));
          }
          const defaultArm =
        ...
        setFormArms(teacherFormArms);
        setCardState(initial);
        setClassSizes(Object.fromEntries(sizeEntries));
        setError("");
      } catch (e) {
        if (!cancelled) {
          setError(e instanceof Error ? e.message : "Failed to load form class results");
        }
      } finally {
  ...
  }, [isTeacher, isAdminish]);

  const visibleLevels = useMemo(() => {
    if (!isTeacher || isAdminish) return levels;
    const levelIds = new Set(
      arms.filter((a) => formArms.includes(a.id)).map((a) => a.class_level),
    );
    return levels.filter((l) => levelIds.has(l.id));
  }, [isTeacher, isAdminish, levels, arms, formArms]);
  ...
  function updateCard(levelId: number, patch: Partial<CardState>) {
    ...
    if (patch.armId) void refreshClassSize(Number(patch.armId));
  }
  ...
  if (loading) {
    return <p className="text-sm text-[var(--muted)]">Loading form class results…</p>;
  }
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-3 max-w-2xl text-base text-[var(--muted)]">
          Choose class, term, and arm (default A), then manage form-class remarks,
          attendance, and the default assessments (
          {FORM_CLASS_ASSESSMENTS.map((a) => a.label).slice(0, 3).join(", ")}
          …).
        </p>
      </header>
      ...
      {visibleLevels.length === 0 ? (
        <p className="rounded-2xl border border-[var(--line)] bg-white/90 px-5 py-8 text-sm text-[var(--muted)]">
          {isTeacher
            ? "No form-class assignment found. Ask an admin to set you as Form Teacher for a class arm."
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {visibleLevels.map((level) => {
            ...
            let levelArms = arms.filter((a) => a.class_level === level.id);
            if (isTeacher && !isAdminish) {
              levelArms = levelArms.filter((a) => formArms.includes(a.id));
            }
            ...
            const manageHref =
              state.termId && state.armId
                ? `/app/results/form-class/${level.id}?term=${state.termId}&arm=${state.armId}`
                : null;
              ...
              >
                ...
                <div className="border-t border-[var(--line)] bg-[#fff3e8] px-4 py-3 text-sm">
                  ...
                  <div className="mt-2 grid grid-cols-3 gap-1 border border-[var(--line)] bg-white/80 p-2 text-center text-[0.65rem] font-semibold uppercase tracking-wide">
                    {FORM_CLASS_ASSESSMENTS.slice(0, 3).map((item) => (
